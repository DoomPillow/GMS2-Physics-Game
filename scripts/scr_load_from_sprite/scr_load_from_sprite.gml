...
function load_object_from_sprite(_spr,_submg)
{
	...
	for (var _y = 0; _y < _h; ++_y) {
			for (var _x = 0; _x < _w; ++_x) {
					...
					switch (_col) {
						case 16777215:
							array_push(components, instance_create_layer(x,y,layer,obj_recungus, {color: _col, collision_layer: 0, xoff: _x * 32, yoff: -_y * 32 , vertex: [new vector(x, y), new vector(x+32, y)], width: 32}));
							mass += 6;
						break;
						case 7368816:
							array_push(components, instance_create_layer(x,y,layer,obj_recungus, {color: _col, collision_layer: 1, xoff: _x * 32, yoff: -_y * 32 , vertex: [new vector(x, y), new vector(x+32, y)], width: 32}));
							mass += 1;
						break;
					}
			}
